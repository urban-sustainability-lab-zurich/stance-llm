...
def allowed_openai_chains():
    allowed = [chain for chain in REGISTERED_LLM_CHAINS if chain not in CONSTRAINED_GRAMMAR_CHAINS]
    return(allowed)

# non-masked, iterate across all chains

@pytest.fixture(scope="module")
def stance_detection_runs_openai(test_examples,
                                gpt35_openai,
                                allowed_openai_chains):
    classifications = []
